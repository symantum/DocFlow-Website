...
const highlights = [
  ...
  { icon: Globe,       value: '24/7',  label: 'Global Support',               desc: 'Strategically located teams in Australia and China providing round-the-clock coverage.' },
            ...
            <div key={h.label} className="feature-card p-6">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-sky-100 rounded-xl flex items-center justify-center">
                  <h.icon size={18} className="text-sky-600" />
                </div>
                <span className="text-2xl font-black text-slate-900">{h.value}</span>
              </div>
              <p className="text-sm font-semibold text-slate-200 mb-1">{h.label}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          <div>
            ...
            <div className="space-y-4 text-sm text-slate-500 leading-relaxed">
              <p>Symantum was founded with a single mission: deliver operational excellence in document processing so that businesses could focus on what they do best. Over twenty years, we built deep expertise across financial services, government agencies, and educational institutions — processing millions of documents with accuracy and security at the core.</p>
              <p>As AI and cloud technology matured, we made a deliberate transformation. The same methodologies that drove 70% cost reductions for BPO clients are now embedded in our BPaaS platform — accessible to any business, at any scale, as a subscription service.</p>
              <p>Today, AP Portal and CSA Portal represent the culmination of that journey: purpose-built software that ingests invoices from email, extracts every field with AI-validated precision, and pushes clean data straight into your accounting system — all with a human reviewer in the loop.</p>
            </div>
          </div>
              ...
              {timeline.map((t, i) => (
                <div key={i} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    ...
                    {i < timeline.length - 1 && <div className="w-px flex-1 bg-sky-600/20 my-1" />}
                  </div>
                  <div className="pb-5">
                    <span className="text-xs font-bold text-sky-600 uppercase tracking-wider">{t.year}</span>
                    <p className="text-sm text-slate-500 mt-0.5 leading-relaxed">{t.event}</p>
                  </div>
                </div>
              ))}
            </div>
