...
const stack = [
  {
    phase: 'Ingestion Layer', color: 'text-blue-400', border: 'border-blue-400/20', bg: 'bg-blue-400/[0.04]',
    items: [
  ...
  {
    phase: 'Extraction Layer', color: 'text-cyan-400', border: 'border-cyan-400/20', bg: 'bg-cyan-400/[0.04]',
    items: [
  ...
  {
    phase: 'Validation Layer', color: 'text-violet-400', border: 'border-violet-400/20', bg: 'bg-violet-400/[0.04]',
    items: [
  ...
  {
    phase: 'Integration Layer', color: 'text-emerald-400', border: 'border-emerald-400/20', bg: 'bg-emerald-400/[0.04]',
    items: [
  ...
  {
    phase: 'Infrastructure', color: 'text-slate-400', border: 'border-slate-400/20', bg: 'bg-slate-400/[0.04]',
    items: [
    ...
    <div className="pt-28 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl sm:text-5xl font-black text-white mb-4">Built for precision at scale</h1>
        ...
        {/* Pipeline diagram */}
        <div className="glass-card p-8 mb-14 overflow-x-auto">
          <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-blue-400 mb-6 text-center">Full Pipeline Architecture</p>
              ...
              <div key={node.label} className="flex items-center flex-1">
                ...
                {i < arr.length - 1 && (
                  <div className="w-4 flex items-center justify-center shrink-0">
                    ...
                    <div className="w-0 h-0 border-l-[4px] border-l-slate-700 border-y-[3px] border-y-transparent" />
                  </div>
                )}
              </div>
            ))}
              ...
              <div className="px-8 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                {layer.items.map(item => (
                  <div key={item.name} className="flex gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-200 mb-1">{item.name}</p>
        ...
        <div className="mt-14 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {integrations.map(i => (
              <span key={i} className="px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/[0.08] text-sm text-slate-400 font-medium">
                {i}
              </span>
            ))}
          </div>
